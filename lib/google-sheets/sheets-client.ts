...
export async function crearPestanaPlantilla({
  ...
  pacientes: any[]
}): Promise<{ url: string; nombreHoja: string }> {
  const auth = getAuth()
  const sheets = google.sheets({ version: 'v4', auth })

  // Intento optimista: como la API ya valida duplicados (transportista + fecha_ruta)
  // antes de llegar aqui, en el caso normal esto funciona a la primera - sin gastar
  // una llamada extra solo para "preguntar" si el nombre ya existe.
  ...
  try {
    const addSheetRes = await sheets.spreadsheets.batchUpdate({
      spreadsheetId,
      requestBody: { requests: [{ addSheet: { properties: { title: nombreFinal } } }] },
    })
    sheetId = addSheetRes.data.replies?.[0]?.addSheet?.properties?.sheetId
  } catch (e: any) {
    const yaExiste = e?.message?.includes('already exists')
    if (!yaExiste) throw e

    // Solo si de verdad choco un nombre (caso raro), se consultan los existentes
    // y se reintenta una vez con sufijo.
    ...
    let contador = 2
    while (titulos.has(`${nombreHoja} (${contador})`)) contador++
    nombreFinal = `${nombreHoja} (${contador})`

    const reintento = await sheets.spreadsheets.batchUpdate({
      spreadsheetId,
      requestBody: { requests: [{ addSheet: { properties: { title: nombreFinal } } }] },
    })
    sheetId = reintento.data.replies?.[0]?.addSheet?.properties?.sheetId
  }
  ...
  // Escribir los valores y aplicar el formato NO dependen entre si - antes iban
  // en fila, ahora corren al mismo tiempo.
  await Promise.all([
    sheets.spreadsheets.values.update({
      spreadsheetId,
      range: `'${nombreFinal}'!A1`,
    ...
    sheets.spreadsheets.batchUpdate({
      ...
      requestBody: {
        requests: [
          ...
          // 6. Filas 5 en adelante: Datos (fondo blanco)
          {
            repeatCell: {
